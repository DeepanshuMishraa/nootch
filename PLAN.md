...
## Context
- Agent Notch already renders provider usage rings, but it does not show whether a local coding agent is working, waiting for user action, finished, or idle.
...
## Approach
- First map Herdr's detection pipeline and status semantics from its source, then map those states onto Agent Notch's existing provider model.
...
## Files to modify
- `Sources/AgentNotch/Domain.swift`: typed activity state, agent identity, and provider-level activity projection.
- `Sources/AgentNotch/AgentActivityDetector.swift`: macOS-local process/activity polling and conservative agent-specific evidence rules, independent of Herdr.
- `Sources/AgentNotch/UsageStore.swift`: merge activity snapshots with quota refreshes and expose a separate activity refresh cadence.
- `Sources/AgentNotch/AgentNotchApp.swift`: start and stop the activity monitor with the app lifecycle.
- `Sources/AgentNotch/NotchPanel.swift`: spin only working rings, add a distinct needs-action treatment, and leave done/idle rendering unchanged.
- `Tests/AgentNotchTests/AgentNotchTests.swift` or focused new test files: detector mapping, state transitions, provider mapping, and presentation decisions.

## Reuse
- Existing `UsageStore` owns refresh state and cached provider snapshots in `Sources/AgentNotch/UsageStore.swift`; quota fetching and activity polling should remain separate so a slow API call cannot make an agent look busy.
- Existing `ProviderStatus` in `Sources/AgentNotch/Domain.swift` should carry the typed activity projection rather than view-only string flags.
...
- Existing `ProviderRailItem` in `Sources/AgentNotch/NotchPanel.swift` owns the compact ring and is the only place that needs motion/attention presentation logic.
...
- [x] Inspect current Agent Notch models, store, panel, ring views, and tests.
